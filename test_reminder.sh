...
startmessage="[DBG] `date "+%a %b %d %T %N %Z %Y"`: NEW START: ams_ip=$ams_ip, count_reminders=$param, count_iterations=$count_iterations, RACK_DATE=( ${RACK_DATA[@]} ), RACK_CHANNELS=( ${RACK_CHANNELS[@]} )"
...
echo $startmessage|$logwrap
for (( i=1; i<=$count_iterations; i++)); do for reminderProgramStart in ${RACK_DATE[@]}; do for reminderChannelNumber in ${RACK_CHANNELS[@]}; do
$reminder $action $Offset;  sleep 1; done; done; done
...
echo $startmessage|$logwrap
for (( i=1; i<=$count_iterations; i++)); do for reminderProgramStart in ${RACK_DATE[@]}; do for reminderChannelNumber in ${RACK_CHANNELS[@]}; do
 $reminder $action $Offset $Offset_new; done; done; done
